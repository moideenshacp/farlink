...
class AbsenteeCheckJob {
  async runDailyAbsenteeCheck() {
    ...
    try {
      ...
      await this.sendAlert("Success", "Daily absentee check completed successfully.");

    } catch (error:any) {
      console.error("Error during absentee marking:", error);
      await this.sendAlert("Failure", `Error in absentee check: ${error.message}`);
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
          ...
          <h2 style="text-align: center; color: ${status === "Success" ? "green" : "red"};">Cron Job Status: ${status}</h2>
          <p style="text-align: center;">The scheduled cron job has completed execution.</p>
          <p style="text-align: center;">Details:</p>
          <blockquote style="background: #f9f9f9; padding: 10px; border-left: 5px solid ${status === "Success" ? "green" : "red"};">
            ${message}
          </blockquote>
          <p style="text-align: center; font-size: 12px; color: #555;">Executed at: ${new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</p>
        </div>
      `,
    });
  
    console.log(`Cron Job Alert Sent: ${message}`);
  }
  
}
export const jobStart = cron.schedule("25 16 * * *", async () => {  
    try {
